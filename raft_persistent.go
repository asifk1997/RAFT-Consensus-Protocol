...
func initializeLog() {
	var file *os.File
	if _, err := os.Stat(myAddress+"_info.log"); err == nil {
		// path/to/whatever exists
		// if empty write zero
		fmt.Println("file exists")
		//incrementTerm()
		file,err = os.Open(myAddress+"_info.log")
		check(err)
	} else if os.IsNotExist(err) {
		// path/to/whatever does *not* exist
		file, err = os.Create(myAddress+"_info.log")
		if err!= nil {
			log.Fatal(err)
		}
		...
		term = 0
		log.Println("Created log file and init term 0")

	} else {
		// Schrodinger: file may or may not exist. See err for details.
		// Therefore, do *NOT* use !os.IsNotExist(err) to test for file existence
		panic("dont know what to do in this case")
	}
	defer file.Close()
	initializeTermFile()
}
...
func initializeTermFile() {
	if _, err := os.Stat(myAddress+"_info_term.log"); err == nil {
		// path/to/whatever exists
		// if empty write zero
		//fmt.Println("file exists")
		//incrementTerm()
		input, err := ioutil.ReadFile(myAddress+"_info_term.log")
		if err != nil {
			log.Fatalln(err)
		}
		termString,err:= strconv.Atoi(string(input))
		check(err)
		term = termString
	} else if os.IsNotExist(err) {
		// path/to/whatever does *not* exist
		file2, err := os.Create(myAddress+"_info_term.log")
		if err!= nil {
			log.Fatal(err)
		}
		_,err = file2.WriteString("0")
		//_, err := file2.WriteAt([]byte{'0'}, 0) // Write at 0 beginning
		if err != nil {
			log.Fatalf("failed writing to file: %s", err)
		}
		term = 0
		///log.Println("Created file and init term 0")
		defer file2.Close()
	} else {
		// Schrodinger: file may or may not exist. See err for details.
		// Therefore, do *NOT* use !os.IsNotExist(err) to test for file existence
...
func incrementTerm() {
	...
	file2,err := os.OpenFile(myAddress+"_info_term.log",os.O_CREATE | os.O_RDWR,0666)
	if err!=nil{
		log.Fatal(err)
	}
	...
	y++
	s2 := strconv.Itoa(y)
	_, err = file2.WriteAt([]byte(s2), 0) // Write at 0 beginning
	if err != nil {
		log.Fatalf("failed writing to file: %s", err)
	}
	term = y
	//fmt.Println("current term ",term)
	defer file2.Close()
}
...
func getNumberOfLines() (int) {
	//fmt.Println("call to get number of  lines")
	file2,err := os.OpenFile(myAddress+"_info.log",os.O_CREATE | os.O_RDWR,0666)
	if err!=nil{
		log.Fatal(err)
	}
	defer file2.Close()
...
func addToMyLog(line string,n int) {
	f, err := os.OpenFile(myAddress+"_info.log", os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		log.Println(err)
	}
	defer f.Close()
	myLog := "\n"+strconv.Itoa(n)+"."+strconv.Itoa(term)+"."+line
	if _, err := f.WriteString(myLog); err != nil {
		log.Println(err)
	}
}
...
func takeActionOnMessage(body []byte) {
	...
	if strings.HasPrefix(s,"requestVote"){
		...
		if (leadersTerm < term){
			...
			myLogArray := getCompleteLogArray()

			if ((len(myLogArray)-1) < startPointIndex){
				//fmt.Println("case 2")
				msg := "reply_"+strconv.Itoa(term) +"_" +strconv.Itoa(lengthLeaderEntriesArray)+"_"+myAddress+"_"+"0"
				sendMessageToPeer(msg,leaderAddressString)
				...
				for i:=startPointIndex;i<=endPointIndex;i++{
					if (myLogArray[startPointIndex] == leaderEntriesArray[i-startPointIndex]){
						matches=true
						break
					}
				}
				...
				if matches==true{
					...
					initialLengthOfMyLogArray := len(myLogArray)
					for i:=startPointIndex;i<=endPointIndex ;i++{
						...
						if initialLengthOfMyLogArray -1 < i{
							...
							myLogArray = append(myLogArray,leaderEntriesArray[i-startPointIndex])
						}else{
							//fmt.Println("myLogArray1",myLogArray[i],leaderEntriesArray[i-startPointIndex])
							myLogArray[i] = leaderEntriesArray[i-startPointIndex]
						}
					}
					myLogArray = myLogArray[0:endPointIndex+1]
					//fmt.Println("After myLogArray",myLogArray)
					// write back to log file
					output := strings.Join(myLogArray, "\n")
					err = ioutil.WriteFile(myAddress+"_info.log", []byte(output), 0644)
					if err != nil {
						log.Fatalln(err)
					}
